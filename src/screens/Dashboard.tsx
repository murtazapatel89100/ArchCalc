...
import { Search, Settings, Moon, Sun, Terminal, Copy, Check, Command } from "lucide-solid";
import { Transition } from "solid-transition-group";
import { cn } from "../utils/cn";
import { evaluator, EvalResult } from "../utils/evaluator";
...
export function Dashboard() {
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === "Enter" && res && res.type !== "error") {
      // Save to history
      const now = new Date();
      const newItem: HistoryItem = {
        id: crypto.randomUUID(),
        title: getTitleForQuery(q, res.type),
        expression: q,
        result: res.value,
        tags: [res.type === "calc" ? "Calculation" : res.type === "conv" ? "Conversion" : "Developer"],
  ...
  const getTitleForQuery = (q: string, type: string) => {
    ...
    if (q.toLowerCase().includes("now") || q.toLowerCase().includes("timestamp")) return "Timestamp";
        ...
        <div class="w-full max-w-3xl flex flex-col gap-8 mt-10">
          ...
          <div 
            class={cn(
              ...
              isFocused() ? "border-[var(--color-app-accent)] shadow-[var(--color-app-accent)]/10" : "border-[var(--color-app-border)] shadow-black/20"
            )}
          >
            <div class="flex items-center px-6 py-5">
              <Terminal size={24} class={cn("mr-4 transition-colors", isFocused() ? "text-[var(--color-app-accent)]" : "text-[var(--color-app-text-secondary)]")} />
              <input
                ref={inputRef}
                type="text"
                value={query()}
            ...
            <Transition 
              onEnter={(el, done) => {
                const a = el.animate([{ height: 0, opacity: 0 }, { height: el.scrollHeight + 'px', opacity: 1 }], { duration: 200 });
                a.finished.then(done);
              }}
              onExit={(el, done) => {
                const a = el.animate([{ height: el.scrollHeight + 'px', opacity: 1 }, { height: 0, opacity: 0 }], { duration: 200 });
                a.finished.then(done);
              }}
            >
                ...
                <div class="border-t border-[var(--color-app-border)] overflow-hidden">
                  <div class="px-6 py-5 flex items-center justify-between bg-[var(--color-app-surface-secondary)]/30 rounded-b-2xl">
                    <div class="flex flex-col flex-1 min-w-0 mr-4">
                      <span class="text-sm font-mono text-[var(--color-app-text-secondary)] mb-1">Result</span>
                      <span class={cn(
                        "text-3xl font-semibold tracking-tight truncate",
                        isError() ? "text-[var(--color-app-error)]" : "text-[var(--color-app-success)]"
                      )}>
                        {resultText()}
                      </span>
                    </div>
                    <Show when={!isError()}>
                      ...
                      >
                        <Show when={copied()} fallback={<Copy size={18} />}>
                           <Check size={18} class="text-[var(--color-app-success)]" />
                        </Show>
          ...
          >
            <h3 class="text-sm font-medium text-[var(--color-app-text-secondary)] px-2">Recent & Suggestions</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                { label: "100 gb in mb", desc: "Data Conversion", type: "conv" },
                ...
                >
                  <div class="flex flex-col">
                    <span class="text-base font-medium text-[var(--color-app-text-primary)] group-hover:text-[var(--color-app-accent)] transition-colors">{item.label}</span>
                    <span class="text-xs text-[var(--color-app-text-secondary)] mt-1">{item.desc}</span>
                  </div>
